...
@Component({
  selector: 'app-gru-solar-chart',
  templateUrl: './gru-solar-chart.component.html',
  styleUrls: ['./gru-solar-chart.component.scss']
})
export class GRUSolarChartComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.availableModels = await this.httpClient.get<string[]>(environment.webserverUrl + "getAvailableModels/gru-solar").toPromise();
  ...
  async startPredicting(){
    ...
    const url = environment.webserverUrl + "gru-solar/" + this.selectedModel + "/model.json";
    ...
    let results: TimeSeries = await this.predictValues(this.GRUModel, this.groundTruth, this.solarIrradiance, startDate, endDate);
  ...
  async startPredictingAllGRUMOdels(){
    for(let model of this.availableModels){
      ...
      this.GRUModel = await tf.loadLayersModel(environment.webserverUrl + "gru-solar/" + model + "/model.json").catch(_ => {
        this._snackBar.open("GRU model could not be loaded. Is the fileserver running?");
      }) as LayersModel;
  ...
  async predictValues(model: tf.LayersModel, groundTruth: TimeSeries, solarIrradiance: TimeSeries, startDate: string, endDate: string): Promise<TimeSeries> {
    let predictor = new GRUSolarPredictor(model);
 ...
 /*### chart 2 ### */
  ...
  updateGRULiveChart(){
    ...
    let predictor = new GRUSolarPredictor(this.GRUModel);
